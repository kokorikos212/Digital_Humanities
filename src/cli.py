...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    parser.add_argument(
        ...
        help="Export the RDF graph as an interactive HTML file",
    )
    parser.add_argument(
    ...
    if args.visualize:
        ...
        ttl_files = sorted(config.rdf_dir.glob("ontology_*.ttl"), reverse=True)
        if ttl_files:
            ttl_data = ttl_files[0].read_text(encoding="utf-8")
            try:
                graph_html = render_rdf_graph(ttl_data, height="650px")
                ts = int(time.time())
                graph_path = config.graphs_dir / f"graph_{ts}.html"
                ...
                print(f"\n  ⚠️  Graph export failed: {exc}")
        else:
            print("\n  ⚠️  No RDF data found — run with triple generation enabled.")
