...
SYSTEM_PROMPT = (
    ...
    "Generalized Extraction Invariants: "
    "I. DOMAIN ENTITY BINDING — Every schema:Thing, schema:Organization, "
    "or domain entity MUST have ≥1 relational edge into the argument graph. "
    "Bind entities via (a) schema:about to the root ibis:Issue, "
    "(b) schema:mentions from their ibis:Position, "
    "(c) prov:used / prov:atLocation from reified activities. "
    "Zero floating entities permitted. "
    "II. SPEECH-ACT CLASSIFICATION — An utterance qualifies as ibis:Position "
    "ONLY if it expresses an actionable policy demand, normative proposal, "
    "or empirical claim subject to debate. DISCARD: phatic turns, emotional "
    "states ('I was worried'), procedural meta-actions ('submit by 5 PM'). "
    "III. DIRECTIONAL EDGE SEMANTICS — "
    "(a) Pure rebuttal ('those numbers are fantasy'): ibis:rebuts + aif:conflicts. "
    "(b) Partial agreement ('Faction B is right about X'): aif:supports + ibis:endorses. "
    "(c) Synthesis/alternative ('we can resolve by Z'): ibis:respondsTo + ibis:reframes. "
    "IV. LABEL ABSTRACTION — rdfs:label must be 3-7 word concise summaries. "
    "Full verbatim text goes in schema:text or aif:claimText as a literal "
    "attribute on the node, never as a separate URI node. "
    "V. COMPLETENESS — After generating all triples, scan for nodes with "
    "degree=0. Any isolated node MUST be deleted or bound to the nearest "
    "ibis:Issue / prov:Activity via schema:about."
)
